...
class Installer {
	...
	inject() {
		...
		var lr = readline.createInterface({
			input: fs.createReadStream(`${config.paths.installPath}/resources/app/app_bootstrap/index.js`)
		});
		...
		lr.on('close', () => {
			fs.writeFileSync(`${config.paths.installPath}/resources/app/app_bootstrap/index.js`, lines.join('\n'));
